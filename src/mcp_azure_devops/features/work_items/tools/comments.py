...
def _format_comment(comment) -> str:
    ...
    if hasattr(comment, "created_by") and comment.created_by:
        if (
            hasattr(comment.created_by, "display_name")
            and comment.created_by.display_name
        ):
...
def _get_project_for_work_item(
    item_id: int, wit_client: WorkItemTrackingClient
) -> Optional[str]:
    """
...
def _add_work_item_comment_impl(
    ...
    new_comment = wit_client.add_comment(
        request=comment_request, project=project, work_item_id=item_id
    )
...
def register_tools(mcp) -> None:
    ...
    @mcp.tool()
    def get_work_item_comments(id: int, project: Optional[str] = None) -> str:
        """
    ...
    def add_work_item_comment(
        id: int, text: str, project: Optional[str] = None
    ) -> str:
        """
        ...
        Args:
            id: The work item ID (integer). Example: 502199
                This should be a positive integer representing the unique
                identifier of the work item in Azure DevOps.
            text: The text of the comment. Supports markdown format (recommended), 
                plain text, or HTML. Markdown will be automatically converted to HTML 
                for Azure DevOps. Supported markdown: headers (#), bold (**text**), 
                italic (*text*), code (`code`), links ([text](url)), and lists (- item).
            project: Optional project name. If not provided, will be
                determined from the work item.
