...
def _build_field_document(
    fields: Dict[str, Any], operation: str = "add"
) -> list:
    """
    ...
    for field_name, field_value in fields.items():
        ...
        document.append(
            JsonPatchOperation(op=operation, path=path, value=field_value)
        )
...
def _create_work_item_impl(
    ...
    Args:
        ...
        acceptance_criteria: Optional acceptance criteria. Supports markdown format (recommended), plain text, or HTML. Markdown will be automatically converted to HTML for Azure DevOps. Supported markdown: headers (#), bold (**text**), italic (*text*), code (`code`), links ([text](url)), and lists (- item).
    ...
    if acceptance_criteria:
        ...
        new_work_item = wit_client.update_work_item(
            document=ac_document, id=new_work_item.id, project=project
        )
    ...
    try:
        ...
        if parent_id:
            ...
            new_work_item = wit_client.update_work_item(
                document=link_document, id=new_work_item.id, project=project
            )
        # Related links
        if related_ids:
...
def _update_work_item_impl(
    ...
    if document:
        updated_work_item = wit_client.update_work_item(
            document=document, id=id, project=project
        )
    else:
    ...
    if related_ids:
        ...
        for related_id in related_ids:
            ...
            updated_work_item = wit_client.update_work_item(
                document=related_document, id=id, project=project
            )
...
def _add_link_to_work_item_impl(
    ...
    updated_work_item = wit_client.update_work_item(
        document=link_document, id=source_id, project=project
    )
...
def register_tools(mcp) -> None:
    ...
    def create_work_item(
        ...
        Args:
            ...
            fields: Optional dictionary of additional field name/value pairs
                to set
            description: Optional description of the work item. Supports markdown format (recommended), plain text, or HTML. Markdown will be automatically converted to HTML for Azure DevOps. Supported markdown: headers (#), bold (**text**), italic (*text*), code (`code`), links ([text](url)), and lists (- item).
            state: Optional initial state for the work item
            assigned_to: Optional user email to assign the work item to
    ...
    def update_work_item(
        ...
        Args:
            ...
            description: Optional new description. Supports markdown format (recommended), plain text, or HTML. Markdown will be automatically converted to HTML for Azure DevOps. Supported markdown: headers (#), bold (**text**), italic (*text*), code (`code`), links ([text](url)), and lists (- item).
